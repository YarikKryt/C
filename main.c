...
int main(){
  ...
  while(match == 0 && chance_count <= MAX_CHANCES/*as long as match is false and chances are <= MAX_CHANCES*/){ // 20 Marks
    ...
    if (user_guess > computer_guess/* write the condition */){                     // 10 Marks
      ...
      printf("Remaining chances are: %d,", (MAX_CHANCES - chance_count));
    }
  ...
  if (match = 0 && chance_count > 7){                // 10 marks
      printf("Game over!!! Try to be Mathemetical.");// print the game over message.                 // 5 marks
  }
  return 0;
}
